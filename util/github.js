...
function _getCommitSHAsToBranchOfRepo(repo_name, branch_name) {
    ...
    return github.repos.getCommits({
        ...
        commits.forEach(function(commit) {
            console.log("Commit SHA hit: " + repo_name + " :: " + branch_name + " :: " + commit.sha);
...
function _getCommitsToBranchOfRepo(repo_name, branch_name) {
    var result = [];
    return _getCommitSHAsToBranchOfRepo(repo_name, branch_name).then(function(shas) {
        var plist = [];
        shas.forEach(function(sha) {
            console.log(cli_border);
            console.log(">> fetching commit of: " + repo_name + " :: " + branch_name + " :: " + sha);
            plist.push(github.repos.getCommit({
                owner: org_id,
                repo: repo_name,
                sha: sha
            }).then(function(commit) {
                console.log("Commit hit: " + repo_name + " :: " + branch_name + " :: " + sha);
                result.push(commit);
            }));
        });
...
function _getLinesCommittedToRepo(repo_name) {
    ...
    return _getBranchNames(repo_name).then(function(branchNames) {
        ...
        branchNames.forEach(function(branch_name) {
            console.log(">> processing branch: " + repo_name + " :: " + branch_name);
            plist.push(_getCommitsToBranchOfRepo(repo_name, branch_name).then(function(commits) {
                commits.forEach(function(commit) {
                    commit = commit.data || {};
                    var username = "NULL";
                    ...
                    console.log(">> processing commit:" + repo_name + " :: " + branch_name + " :: " + commit.sha + " :: " + username + ": " + numLines);
                    if (username in result) result[username] += numLines;
...
function _getLinesCommittedToEachRepo() {
    ...
    return github.repos.getForOrg({
        ...
        repos.forEach(function(repo) {
            ...
            plist.push(_getLinesCommittedToRepo(repo.name).then(function(usernameToLines) {
